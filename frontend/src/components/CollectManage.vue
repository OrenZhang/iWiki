<template>
    <div class="collect-manage">
        ...
        <el-table size="medium" :data="docs" v-show="!loading" :show-header="false">
            ...
            <el-table-column :label="$t('creator')" prop="creator_name" width="200px" />
            <el-table-column :label="$t('updateAt')" prop="update_at" width="200px" />
            <el-table-column :label="$t('operation')" width="160px">
                <template #default="scope">
                    <el-button type="text" @click="goTo('doc/' + scope.row.doc_id)">
                        {{ $t('open') }}
                    </el-button>
                    <el-button type="text" @click="showCancelCollectConfirm(scope.row)">
                        {{ $t('cancelCollect') }}
                    </el-button>
                </template>
            </el-table-column>
...
<script setup>
    ...
    import { deleteDocAPI, listDocManageAPI, loadCollectDocAPI, unCollectDocAPI } from '../api/modules/doc'
